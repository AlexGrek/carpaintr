({
    ...
    run: (x, carPart, tableData, repairAction, files, carClass, carBodyType, carYear, carModel, paint) => {
        ...
        // - row clause section -
        output.push(mkRow({name: "Зняти «Деталь» для ремонту", evaluate: tableData["Арматурные работы"]["СНЯТИЕ ДЛЯ РЕМОНТА"], trace: traceRowToTable("Арматурные работы", "СНЯТИЕ ДЛЯ РЕМОНТА"), tooltip: "Арматурные работы снять"}));
        output.push(mkRow({name: "Встановити «Деталь» після ремонту", evaluate: tableData["Арматурные работы"]["УСТАНОВКА ДЛЯ РЕМОНТА"], trace: traceRowToTable("Арматурные работы", "УСТАНОВКА ДЛЯ РЕМОНТА"), tooltip: "Just paint part"}));
