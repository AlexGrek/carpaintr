...
const NamedItemEditor = ({ name, value, onNameChange, onValueChange, onDelete }) => {
    return (
        <Panel bordered shaded style={{ marginBottom: '1rem' }}>
            <Stack justifyContent="space-between" alignItems="center" style={{ marginBottom: '1rem' }}>
                <Stack.Item grow={1}>
                    <Input 
                        value={name} 
            ...
            <ItemEditor value={value} onChange={newValue => onValueChange(name, newValue)} />
        </Panel>
    );
};
