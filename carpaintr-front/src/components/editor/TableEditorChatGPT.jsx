...
const TableEditorChatGPT = ({ open, onClose, onSave, fileName, csvData }) => {
  ...
  const renderEditPopover = (rowIndex) => (
    <Popover full>
      <Dropdown.Menu>
        <Dropdown.Item onClick={() => addRow(rowIndex)}>
          <Trans>Add Row Above</Trans>
        </Dropdown.Item>
        <Dropdown.Item onClick={() => addRow(rowIndex + 1)}>
          <Trans>Add Row Below</Trans>
        </Dropdown.Item>
        <Dropdown.Item onClick={() => addRow(rowIndex + 1, true)}>
  ...
  const renderCell = (rowData, rowIndex, colKey) => {
    ...
    const isEditing =
      editCell?.row === absoluteRowIndex && editCell?.column === colKey;
    const isSelected =
      selectedCell?.row === absoluteRowIndex && selectedCell?.column === colKey;
      ...
      return (
        <div className="cell-edit-wrapper">
          ...
          <Whisper
            placement="bottomEnd"
            trigger="click"
            speaker={renderEditPopover(absoluteRowIndex)}
          >
