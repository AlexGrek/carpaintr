...
registerTranslations("ua", {
  ...
  "Fix complete, now refresh the page! Done:":
    "Виконано, тепер оновіть сторінку! Знайдено:",
...
const FileEditor = ({
    ...
    const fetchFileContent = async () => {
      ...
      try {
        const response = await authFetch(
          `/api/v1/${fixEndpoint}/${encodeURIComponent(filePath)}`,
        );
        if (!response.ok) {
        ...
        if (isArrayLike(content) && content.length > 0) {
          ...
          setMsg(
            <Notification
              type="warning"
              header={
                str("Fix complete, now refresh the page! Done:") +
                ` ${content.length} errors, ${distinct.length} unique`
              }
            >
              <div
                style={{
          ...
          setMsg(
            <Notification type="success" header={str("Validation result:")}>
              {str("Validation successful, no issues detected")}
            </Notification>,
            { placement: "topEnd" },
          );
        }
        ...
        setMsg(
          <Notification type="error" header={str("Error")}>
            {str("Failed to load file validation result:")} {err.message}
          </Notification>,
          { placement: "topEnd" },
        );
      } finally {
  ...
  const handleValidate = useCallback(() => {
    const fetchFileContent = async () => {
      ...
      try {
        const response = await authFetch(
          `/api/v1/${validateEndpoint}/${encodeURIComponent(filePath)}`,
        );
        if (!response.ok) {
          throw new Error(str(`Bad response: ${response.statusText}`));
        }
        ...
        if (isArrayLike(content) && content.length > 0) {
          ...
          setMsg(
            ...
            >
              ...
              <br />
              <Button appearance="ghost" onClick={handleFix}>
                <Trans>Try to fix on server</Trans>
              </Button>
            </Notification>,
          );
        } else {
  ...
  useEffect(() => {
    const fetchFileContent = async () => {
      ...
      try {
        const response = await authFetch(
          `/api/v1/${readEndpoint}/${encodeURIComponent(filePath)}`,
        );
        if (!response.ok) {
        ...
        setMsg(
          <Notification type="error" header={str("Error")}>
            {str("Failed to load file content:")} {err.message}
          </Notification>,
          { placement: "topEnd" },
        );
        setFileContent("");
  ...
  const handleSave = useCallback(
    async (saveValue) => {
      ...
      if (!filePath) {
        setMsg(
          <Notification type="error" header={str("Save Error")}>
            {str("No file path provided to save to.")}
          </Notification>,
          { placement: "topEnd" },
        );
        return;
      ...
      if (validationError) {
        setMsg(
          <Notification type="error" header={str("Validation Error")}>
            {str("Invalid file format:")} {validationError}
          </Notification>,
          { placement: "topEnd" },
        );
        return;
      ...
      if (saveValue === "[object Object]") {
        // Check for accidental object stringification
        setMsg(
          <Notification type="error" header={str("Save Error")}>
            File corrupted by frontend
          </Notification>,
          { placement: "topEnd" },
        );
        return;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          setMsg(
            <Notification type="success" header={str("Success")}>
              {str("File saved")}
            </Notification>,
            { placement: "topEnd" },
          );
        ...
        setMsg(
          <Notification type="error" header={str("Network Error")}>
            {str("Failed to save file:")} {error.message}
          </Notification>,
          { placement: "topEnd" },
        );
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    if (isCommonFile) {
      setMsg(
        <Notification type="warning" header={str("Permission Denied")}>
          {str("Cannot delete common files.")}
        </Notification>,
        { placement: "topEnd" },
      );
      return;
    ...
    if (!filePath) {
      // Cannot delete if there's no file path to begin with
      setMsg(
        <Notification type="warning" header={str("Warning")}>
    ...
    try {
      const response = await authFetch(
        `/api/v1/${deleteEndpoint}/${encodeURIComponent(filePath)}`,
        {
          method: "DELETE",
        },
      );
      if (!response.ok) {
        const errorData = await response.json();
        setMsg(
          <Notification type="error" header={str("Delete Error")}>
      ...
      setMsg(
        <Notification type="error" header={str("Network Error")}>
          {str("Failed to delete file:")} {error.message}
        </Notification>,
        { placement: "topEnd" },
      );
    }
  ...
  const handleDownload = useCallback(() => {
    ...
    a.download = fileName;
    setMsg(
      <Notification type="info" header={str("Download File")}>
        {fileName}
      </Notification>,
      { placement: "topEnd" },
    );
  ...
  return (
    ...
    >
      ...
      <ContentArea>
        ...
        {loadingContent ? (
          <Overlay>
            <Trans>Loading file content...</Trans>
          </Overlay>
        ) : isViewingPreview ? (
          isCSVFile && tableData && tableData.headers.length > 0 ? (
            <TablePreviewContainer>
              ...
              {tableData.hasMore && (
                ...
                >
                  Showing {Math.min(10, tableData.totalRows)} of{" "}
          ...
          <>
            {!isEditing && (
              <Overlay>
                <Trans>Click Edit to modify</Trans>
              </Overlay>
            )}
            <Input
              style={{
              ...
              {isEditing ? (
                <>
                  <Button
                    appearance="primary"
                    onClick={() => handleSave()}
                    disabled={!filePath || !hasUnsavedChanges || loadingContent}
                  >
                ...
                <>
                  ...
                  {validateEndpoint && (
                    <Button
                      ...
                      disabled={isEditing}
                    >
                      <CheckCheck style={{ marginRight: 5 }} />{" "}
              ...
              {filePath &&
                (filePath.endsWith(".yaml") || filePath.endsWith(".yml")) && (
                  <>
                    ...
                    >
                      <ScrollText style={{ marginRight: 5 }} />{" "}
                      <Trans>Open items editor</Trans>
                    </Button>
                    <YamlItemsEditorDrawer
      ...
      >
        <Drawer.Header>
          <Drawer.Title>
            <AlertTriangle color="orange" style={{ marginRight: 10 }} />{" "}
            <Trans>Confirm Deletion</Trans>
          </Drawer.Title>
        </Drawer.Header>
        <Drawer.Body>
          <p>
            <Trans>Are you sure you want to delete the file</Trans>{" "}
