...
} from "../../localization/LocaleContext"; // Import for translations
...
registerTranslations("ua", {
  ...
  "Are you sure you want to revert commit":
    "Ви впевнені, що хочете скасувати зміну",
...
const CommitHistoryDrawer = ({ show, onClose, onRevert }) => {
  ...
  const handleRevertConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMsg(
          <Message type="success" closable>
            <Trans>Commit</Trans> {selectedCommitHash.substring(0, 7)}{" "}
            <Trans>reverted successfully!</Trans>
          </Message>,
          { placement: "topEnd" },
        );
        fetchCommits();
      ...
      setMsg(
        <Message type="error" closable>
          {str("Error reverting commit")} {selectedCommitHash.substring(0, 7)}:{" "}
          {error.message}
        </Message>,
        { placement: "topEnd" },
      );
    ...
    <div>
      ...
      >
        ...
        <Modal.Body>
          ...
          <strong>{selectedCommitHash.substring(0, 7)}</strong>?{" "}
