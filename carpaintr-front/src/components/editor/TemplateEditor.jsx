    ...
    async (saveValue, newName) => {
      ...
      try {
        const response = await authFetch(
          `/api/v1/editor/upload_user_file/${encodeURIComponent(`doc_templates/${newName}`)}`,
          {
            method: "POST",
            body: formData,
          },
    ...
    const fetchData = async () => {
      try {
        ...
        const templateRes = await authFetch(
          `/api/v1/user/get_template/${template}`,
        );
        if (!templateRes.ok)
        ...
        if (!sampleRes.ok)
          throw new Error(`Sample fetch failed: ${sampleRes.status}`);
    ...
    <div className="flex flex-row md:flex-col h-full">
      <div className="flex-1 flex flex-col">
        <Modal
          size="sm"
          backdrop={true}
          open={isSaveAsDrawerOpen}
          onClose={() => setIsSaveAsDrawerOpen(false)}
        >
          <Modal.Footer>
            <Button
              appearance="primary"
              onClick={handleSaveWithNewName}
              disabled={newTemplateName == template}
            >
              <Trans>Save</Trans>
            </Button>
          </Modal.Footer>
