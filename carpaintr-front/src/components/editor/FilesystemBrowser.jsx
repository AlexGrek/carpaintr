...
const FilesystemBrowser = ({ filesystems }) => {
  ...
  const currentFsConfig = useMemo(
    () => filesystems.find((fs) => fs.name === selectedFsName),
    [filesystems, selectedFsName],
  );
  ...
  useEffect(() => {
    ...
    const initialFs =
      filesystems.find((fs) => fs.name === fsFromUrl)?.name ||
      filesystems[0]?.name ||
      null;
    if (initialFs) {
    ...
    async (isRefresh = false) => {
      ...
      try {
        const response = await authFetch(
          `/api/v1/${currentFsConfig.listEndpoint}`,
        );
        ...
        setError(
          "Failed to fetch data. Please check the network and endpoints.",
        );
        console.error(err);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < segments.length - 1; i++) {
      const nextDir = currentLevel.children?.find(
        (c) => "Directory" in c && c.Directory.name === segments[i],
      );
      if (nextDir) {
    ...
    if (pathIsValid) {
      ...
      const fileMatch = currentLevel.children?.find(
        (c) => "File" in c && c.File.name === lastSegment,
      );
      if (fileMatch) {
        setCurrentPath(segments.slice(0, -1));
        setViewingFile(lastSegment);
      } else {
        const dirMatch = currentLevel.children?.find(
          (c) => "Directory" in c && c.Directory.name === lastSegment,
        );
        if (dirMatch) {
  ...
  useEffect(() => {
    if (directoryData) {
      ...
      for (const part of currentPath) {
        const nextDir = currentNavData.children?.find(
          (c) => "Directory" in c && c.Directory.name === part,
        );
        if (nextDir) {
      ...
      if (found && currentNavData?.children) {
        setDisplayedData(
          currentNavData.children.map((child) => {
  ...
  const displayPath = directoryData
    ? `/${directoryData.Directory.name}/${currentPath.join("/")}`.replace(
        /\/\/+/g,
        "/",
      )
    : "/";
  const formattedDisplayPath =
    displayPath.length > 1 && displayPath.endsWith("/")
      ? displayPath.slice(0, -1)
      : displayPath;
  const fullFilePath = viewingFile
    ? [...currentPath, viewingFile].join("/")
    : currentPath.join("/");
  ...
  const handleFileUpload = async (files) => {
    ...
    for (const file of files) {
      ...
      try {
        const response = await authFetch(
          `/api/v1/${currentFsConfig.uploadEndpoint}/${encodeURIComponent(filePath)}`,
  ...
  const handleCreateFile = async () => {
    if (!newFileName || !newFileType || !currentFsConfig?.uploadEndpoint)
      return;
    setLoading(true);
    ...
    const file = new File([content], fileNameWithExt, {
      type: newFileType === "csv" ? "text/csv" : "application/x-yaml",
    });
    const uploadPath = currentPath.join("/");
    const filePath = uploadPath
      ? `${uploadPath}/${fileNameWithExt}`
      : fileNameWithExt;
    const formData = new FormData();
    formData.append("file", file);
    try {
  ...
  const filesystemOptions = filesystems.map((fs) => ({
    label: fs.name,
    value: fs.name,
  }));
  ...
  const renderMenu = () => (
    <>
      <Dropdown.Item eventKey="upload" icon={<FileUp size={16} />}>
        {str("Upload File")}
      </Dropdown.Item>
      <Dropdown.Item eventKey="create-table" icon={<Table size={16} />}>
        {str("Create Table")}
      </Dropdown.Item>
      <Dropdown.Item
        eventKey="create-datasource"
        icon={<FileHeart size={16} />}
      >
        {str("Create Data Source")}
      </Dropdown.Item>
      <Dropdown.Item eventKey="create-proc" icon={<Atom size={16} />}>
        {str("Create Processor")}
      </Dropdown.Item>
    </>
  );
  ...
  return (
    <div onDragEnter={handleDragEnter}>
      ...
      {currentFsConfig?.historyEnabled && (
        <CommitHistoryDrawer
          show={showCommitsDrawer}
          onRevert={() => fetchData(true)}
          onClose={() => setShowCommitsDrawer(false)}
        />
      )}
      ...
      <Modal open={showCreateModal} onClose={() => setShowCreateModal(false)}>
        <Modal.Header>
          <Modal.Title>
            {str(newFileType === "csv" ? "Create Table" : "Create Data Source")}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <InputGroup>
        ...
        <Modal.Footer>
          <Button onClick={handleCreateFile} appearance="primary">
            {str("Create")}
          </Button>
        ...
        >
          ...
          <Dropdown
            ...
            renderToggle={(props, ref) => (
              <IconButton {...props} ref={ref} icon={<Plus />} />
            )}
            placement="bottomEnd"
            trigger={["click"]}
          >
          ...
          {currentFsConfig?.historyEnabled && (
            <IconButton
              icon={<History />}
              onClick={() => setShowCommitsDrawer(true)}
              appearance="subtle"
              style={{ flexShrink: 0 }}
            />
          )}
          ...
          {isDragging ? (
            ...
            <>
              {prevDisplayedData && isAnimating && !viewingFile && (
                <AnimationContainer
                  $animateIn={false}
                  $direction={animationDirection === "right" ? "left" : "right"}
                  $isNew={false}
                >
                  <DirectoryViewTable
              ...
              {viewingFile !== null && currentFsConfig && (
                <AnimationContainer
                  $animateIn={viewingFile !== null && isAnimating}
                  $direction={animationDirection}
                  $isNew={true}
                >
                  {viewingFile && (
                    <FileEditor
                      ...
                      validateEndpoint={
                        currentFsConfig.validateEndpoint || null
                      }
