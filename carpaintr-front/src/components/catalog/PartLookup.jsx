...
const PartLookup = ({ part, onExpand = () => {} }) => {
  ...
  const handleError = useCallback(
    ...
    [str],
  ); // str is not a dependency if it's a static function or imported
  ...
  useEffect(() => {
    ...
    if (part) {
      // Only fetch if 'part' is provided
      fetchData();
    }
  }, [part, handleError]);
      ...
      {data != null &&
        data.map(([tableName, tableData], index) => {
          if (!Array.isArray(tableData) || tableData.length === 0) {
            return (
              <StyledTableContainer key={tableName || `table-${index}`}>
                <h4>
                  <Trans>Table</Trans>{" "}
                  {tableName || `Unnamed Table ${index + 1}`}
                </h4>
                <p>
                  <Trans>No data available for this table.</Trans>
                </p>
          ...
          if (expanded.includes(tableName))
            ...
            return (
              <Panel key={tableName} header={tableName}>
                <p>{columns.join(", ")}</p>
                <IconButton
                  icon={<ArrowDownLine />}
                  onClick={() => setExpanded([...expanded, tableName])}
                >
                  <Trans>Expand</Trans>
                </IconButton>
              </Panel>
