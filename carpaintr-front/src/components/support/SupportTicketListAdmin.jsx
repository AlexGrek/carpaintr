...
export default function SupportTicketListAdmin() {
    ...
    const handleDelete = async (ticket) => {
        ...
        const response = await authFetch(`/api/v1/admin/support_delete?${params.toString()}`, {
            method: 'DELETE',
        });
        if (!response.ok) {
    ...
    return (
        <div className="admin-ticket-list">
            ...
            <div style={{ fontSize: "small" }}>
                {isShowOnlyUnresponded && <p>Showing only unresponded (new) tickets. <IconButton icon={<Plus />} appearance='subtle' onClick={() => setIsShowOnlyUnresponded(false)}>Show all</IconButton></p>}
            </div>
            <Table
                data={tickets}
                autoHeight
                    ...
                    <Cell>
                        {row => (
                            <div className="table-actions">
                                ...
                                <Button size="xs" appearance="subtle" color="red" onClick={async () => await handleDelete(row)}>
                                    DELETE
                                </Button>
                            </div>
                        )}
            ...
            <Drawer size="full" placement="right" open={drawerOpen} onClose={closeDrawer}>
                <Drawer.Header>
