...
export default function SupportTicketView({ ticket, isSupport }) {
    ...
    const sendMessage = async () => {
        ...
        const baseUrl = isSupport ? '/api/v1/admin/support_message' : '/api/v1/user/support_message';
        const params = new URLSearchParams({ email: localTicket.email, id: localTicket.id });
    ...
    const refreshTicket = async () => {
        const params = new URLSearchParams({ email: localTicket.email, id: localTicket.id });
        const res = await authFetch(`/api/v1/user/support_request?${params.toString()}`);
