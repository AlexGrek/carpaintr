...
const passwordModel = Schema.Model({
    currentPassword: StringType().isRequired('Актуальний пароль має бути заповнено.'),
...
const ChangePassword = () => {
    const [formValue, setFormValue] = useState({ currentPassword: '', newPassword: '' });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMessage({ type: 'error', body: errorData.message || 'Помилка зміни паролю.' });
            }
        } catch (error) {
            console.log("error:")
            console.log(error)
            setMessage({ type: 'error', body: 'Помилка, перевірте введений пароль.' });
        } finally {
            console.log("finally")
            setLoading(false);
        }
    ...
    return (
        <>
            {message && <Message type={message.type} closable>{message.body}</Message>}
            <Form
                model={passwordModel}
                formValue={formValue}
