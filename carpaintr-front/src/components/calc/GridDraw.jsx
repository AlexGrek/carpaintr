...
const GridDraw = ({
  ...
  const debouncedOnGridChange = useCallback(
    (newGrid) => {
      ...
      debounceTimeoutRef.current = setTimeout(() => {
        ...
        newGrid.forEach((slice, y) =>
          slice.forEach((value, x) => value > 1 && marked.push(`"${x},${y}"`)),
        );
  ...
  return (
    <div className="grid-container">
      <Divider />
      <p>
        Оберіть тип пошкодження та вкажіть зону пошкодження на схемі.
        <Whisper
          trigger="click"
          placement="bottom"
          ...
          speaker={
            <Tooltip>
              Оберіть тип пошкодження на наступній панелі та відмітьте
              пошкоджені місця відповідним кольором на схемі, приблизно
              зафарбувавши зону пошкодження за допомогою квадратиків нижче.
              Використовуйте інструмент "очистка" для того, щоб очистити
              квадратики. Ви можете використовувати різні типи пошкоджень на
              одній схемі, обираючи різні інструменти. Кожному типу пошкоджень
              відповідає певний колір.
            </Tooltip>
          }
        >
          <Button appearance="link" style={{ display: "inline" }}>
            Як?
          </Button>
        </Whisper>
      </p>
      <div className="grid-controls">
        ...
        >
          ...
          <Radio value={1}>
            <ColorBlock color="yellow" /> Прогин
          </Radio>
          <Radio value={2}>
            <ColorBlock color="orange" /> Складні пошкодження
          </Radio>
          <Radio value={3}>
            <ColorBlock color="red" /> Розрив, складки
          </Radio>
        </RadioGroup>
      </div>
