    ...
    const dataToSave = {
      ...
      body_parts:
        selectedParts.length > 0
          ? selectedParts.map((part) => ({ brand: make || "", model: part }))
          : null,
      vin: VIN,
      notes: notes,
    ...
    async (filename) => {
      try {
        ...
        const response = await authFetch(
          `/api/v1/user/calculationstore?filename=${filename}.json`,
        );
        if (response.ok) {
          const data = await response.json();
          if (data) {
          ...
          showMessage(
            "error",
            `${str("Failed to load calculation:")} ${errorData.message || response.statusText}`,
          );
        }
      } catch (error) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-grow overflow-y-auto p-4">
        ...
        <PanelGroup
          ...
          bordered
          onSelect={(key) => setActivePanel(parseInt(key))}
        >
          <Panel
          ...
          >
            ...
            <Input
              value={licensePlate}
              onChange={handleSetLicensePlate}
