    ...
    try {
      return (
        drawerCurrentPart?.tableData
          ?.find((table) => table.name == "repair_types")
          ?.data["Ремонти"].split("/")
          .map((s) => s.trim()) ?? ["Невідома дія (немає даних)"]
      ).map((i) => ({ label: i, value: i }));
    } catch {
      return (
        ["Невідома дія (помилка)"].map((i) => ({ label: i, value: i })),
        [drawerCurrentPart]
      );
    }
  }, [drawerCurrentPart]);
          ...
          <div className="drawer-tabs-container">
              ...
              >
                <div className="some-padding-sides carousel-page fade-in-expand-simple">
                  ...
                  <MenuPickerV2
                    ...
                    onSelect={(value) =>
                      updateDrawerCurrentPart({ action: value })
                    }
                  />
            ...
            {drawerTab === 1 && (
              ...
              >
                ...
                {(drawerCurrentPart.action.includes("арбу") ||
                  drawerCurrentPart.action === "paint_two_sides") && (
                  <div className="some-padding-sides">
                    <GridDraw
                      gridData={drawerCurrentPart.grid}
                      visual={mapVisual(drawerCurrentPart.name)}
                      onGridChange={(value) =>
                        updateDrawerCurrentPart({ grid: value })
                      }
                    />
                    <SelectionInput
                      ...
                      onChange={(value) =>
                        updateDrawerCurrentPart({ outsideRepairZone: value })
                      }
                    />
                  </div>
                )}
            ...
            {drawerTab === 2 && (
              ...
              >
                ...
                {(drawerCurrentPart.action === "paint_one_side" ||
                  drawerCurrentPart.action === "paint_two_sides") && (
                  <>
                    <p>
                      Зона пошкодження:{" "}
                      {drawerCurrentPart.grid &&
                        drawerCurrentPart.grid
                          .flat()
                    ...
                    <p>
                      ...
                      {
                        outsideRepairZoneOptions[
                          drawerCurrentPart.outsideRepairZone
                        ]
                      }
                    </p>
                  </>
