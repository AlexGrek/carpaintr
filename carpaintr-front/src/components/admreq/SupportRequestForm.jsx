...
registerTranslations("ua", {
  ...
  Submit: "Надіслати",
  "Phone number, viber, telegram, email, etc.":
    "Телефон, вайбер, електронна пошта...",
...
const CustomFileUploader = ({ attachments, setAttachments, disabled }) => {
    ...
    if (!fileExt || !ALLOWED_EXTENSIONS.includes(fileExt)) {
      toaster.push(
        <Message
          type="error"
          closable
        >{`${str("Invalid file type.")}: ${file.name}`}</Message>,
      );
      return false;
    }
    ...
    if (file.size > MAX_SIZE_BYTES) {
      toaster.push(
        <Message
          type="error"
          closable
        >{`${str("File size exceeds the 9MB limit.")}: ${file.name}`}</Message>,
      );
      return false;
    }
          ...
          try {
            ...
            toaster.push(
              <Message
                type="success"
                closable
              >{`${file.name} ${str("uploaded successfully")}`}</Message>,
            );
          } catch (e) {
  ...
  const handleUploadError = (fileId, fileName) => {
    toaster.push(
      <Message
        type="error"
        closable
      >{`${str("File upload failed.")} - ${fileName}`}</Message>,
    );
    setUploadingFiles((prev) => prev.filter((f) => f.id !== fileId));
  };
  ...
  const handleFiles = (files) => {
    ...
    if (
      attachments.length + uploadingFiles.length + fileArray.length >
      MAX_FILES
    ) {
      toaster.push(
        <Message type="error" closable>
          {str("You can upload up to 5 files.")}
        </Message>,
      );
      return;
    }
  ...
  return (
    <div>
      ...
      >
        <FileUpload
          style={{ fontSize: "3em", color: disabled ? "#ccc" : "#3498ff" }}
        />
        <p style={{ marginTop: "8px" }}>
          <Trans>Click or Drag files to this area to upload</Trans>
        </p>
        <small>
          {str("You can upload up to 5 files.")} {str("Max size:")}{" "}
          {MAX_SIZE_MB}MB
        </small>
...
const SupportRequestForm = () => {
      ...
      if (res.ok) {
        toaster.push(
          <Message type="success" closable>
            {str("Support request submitted successfully.")}
          </Message>,
        );
        setSubmitted(true);
        setTitle("");
        ...
        toaster.push(
          <Message
            type="error"
            closable
          >{`${str("Submission failed.")} ${errorData.message || ""}`}</Message>,
        );
      }
    } catch (error) {
  ...
  const types = [
    ...
    "request",
    "proposal",
  ].map((item) => ({ label: str(item), value: item }));

  return (
    <>
      <h2>
        <Trans>Submit a Support Request</Trans>
      </h2>
      <div
        style={{
          maxWidth: "600px",
      ...
      >
        ...
        <Input
          value={title}
          onChange={setTitle}
          placeholder={str("Short summary of the issue")}
        />
        ...
        <TagPicker
          block
          value={reqType.split(" ").filter(Boolean)}
          data={types}
          onChange={(val) => setReqType(val.join(" "))}
          placeholder={str("Choose request type")}
        />
        ...
        {!submitted && (
          <div style={{ marginTop: "20px" }}>
            <Button
              ...
              loading={submitting}
              disabled={!reqType || !title || description.length < 2}
            >
              <Trans>Submit</Trans>
            </Button>
          </div>
        ...
        {submitted && (
          <Message
            showIcon
            type="success"
            header={str("Support request submitted successfully.")}
