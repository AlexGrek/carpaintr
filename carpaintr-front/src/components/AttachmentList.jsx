    ...
    return (
        <div className="attachment-list">
            {attachments.map((id) => {
                ...
                const fileSize = isLoading ? <Loader size="xs" /> : formatSize(meta.size);
                const largeImage = meta && isImage(meta.fileName) && meta.size < 4 * 1024 * 1024;

                return (
                    <div key={id} className={`attachment-item ${largeImage ? 'large-preview' : ''}`}>
                        {largeImage ? (
                            <img
                            ...
                            <div className="attachment-icon" onClick={() => handlePreview(id)}>
                                {meta ? (
                                    isImage(meta.fileName) ? <ImageIcon size={18} /> :
                                        isArchive(meta.fileName) ? <FileArchive size={18} /> :
                                            <FileText size={18} />
                                ) : <FileText size={18} />}
                            </div>
                        )}
            ...
            <Modal open={!!preview} onClose={() => setPreview(null)} size="md">
                ...
                <Modal.Body>
                    {preview?.type === 'image' && (
                        <img src={preview.content} alt={preview.name} style={{ maxWidth: '100%' }} />
                    )}
                    {preview?.type === 'text' && (
                ...
                <Modal.Footer>
                    <Button onClick={() => saveFile(preview.blob, preview.name)} appearance="primary" startIcon={<Download />}>
                        Save
                    </Button>
                    <Button onClick={() => setPreview(null)} appearance="subtle">Close</Button>
                </Modal.Footer>
            </Modal>
        </div>
    );
