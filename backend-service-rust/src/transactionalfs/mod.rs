...
impl GitTransactionalFs {
    ...
    async fn walk_dir_recursive(current_path: &Path) -> Result<Vec<FsEntry>, TransactionalFsError> {
        ...
        children.sort_by(|a, b| {
            let name_a = match a { FsEntry::File { name, .. } | FsEntry::Directory { name, .. } => name };
            let name_b = match b { FsEntry::File { name, .. } | FsEntry::Directory { name, .. } => name };
            name_a.cmp(name_b)
        });
    ...
    async fn run_git_command_with_output(
        ...
        Self::run_git_command_with_output_fallible(cwd, args)
            .await?
            .ok_or_else(|| TransactionalFsError::GitCommand(format!("Git command {:?} unexpectedly returned no output.", args)))
    }
    ...
    async fn run_git_command_with_output_fallible(
        ...
        if !output.status.success() {
            ...
            if args.contains(&"log") && stderr_str.contains("fatal: your current branch") && stderr_str.contains("does not have any commits yet") {
...
mod tests {
    ...
    async fn test_initial_write_and_list() -> Result<(), Box<dyn std::error::Error>> {
        ...
        fs_manager.write_file(file_content.to_vec(), &file_path, commit_message).await?;
    ...
    async fn test_update_file_and_list() -> Result<(), Box<dyn std::error::Error>> {
        ...
        fs_manager.write_file(initial_content.to_vec(), &file_path, "Initial write").await?;
        fs_manager.write_file(updated_content.to_vec(), &file_path, "Update file").await?;
    ...
    async fn test_delete_file_and_list() -> Result<(), Box<dyn std::error::Error>> {
        ...
        fs_manager.write_file(file_content.to_vec(), &file_path, "Add file for deletion").await?;
        assert!(fs_manager.root_path.join(&file_path).exists());

        fs_manager.delete_file(&file_path, "Delete the file").await?;
    ...
    async fn test_delete_non_existent_file() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let result = fs_manager.delete_file(&file_path, "Try to delete non-existent").await;
    ...
    async fn test_revert_last_commit() -> Result<(), Box<dyn std::error::Error>> {
        ...
        fs_manager.write_file(initial_content.to_vec(), &file_path, "Initial commit").await?;
        fs_manager.write_file(updated_content.to_vec(), &file_path, "Update commit").await?;

        let commits_before_revert = fs_manager.list_commits().await?;
        assert_eq!(commits_before_revert.len(), 2);
        assert_eq!(fs::read(fs_manager.root_path.join(&file_path)).await?, updated_content);
    ...
    async fn test_revert_specific_commit() -> Result<(), Box<dyn std::error::Error>> {
        ...
        fs_manager.write_file(b"content1".to_vec(), &file1_path, "Commit A: Add file1").await?;
        fs_manager.write_file(b"content2".to_vec(), &file2_path, "Commit B: Add file2").await?;
        fs_manager.write_file(b"content1_v2".to_vec(), &file1_path, "Commit C: Update file1").await?;

        let commits = fs_manager.list_commits().await?;
        assert_eq!(commits.len(), 3);

        // Find Commit B's hash (which added file2.txt)
        let commit_b_hash = commits.iter()
        ...
        assert_eq!(fs::read(fs_manager.root_path.join(&file1_path)).await?, b"content1_v2");

        let commits_after_revert = fs_manager.list_commits().await?;
        assert_eq!(commits_after_revert.len(), 4); // Original 3 + 1 revert commit
        assert_eq!(commits_after_revert[0].message, format!("Revert \"Commit B: Add file2\""));
